...
vae = AutoencoderKLTemporalDecoder.from_pretrained("/data/yisi/mywork/SV3D-fine-tune/cheeckpoints/stable-video-diffusion-img2vid-xt/vae",torch_dtype=torch.float16, variant="fp16").to("cuda")
...
h = vae.encoder(image.half()).detach()
# pdb.set_trace()
latent_dist = vae.quant_conv(h).detach()
# torch.save(latents_sp, os.path.join(filename, '%03d.pt' % idx))
torch.save(latent_dist, os.path.join(filename, '%03d.pt' % idx))
print("latent size:", latent_dist.shape)
